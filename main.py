...
def load_nomina_data():
    ...
    col_area = first_existing_column(df, ["area", "area_"])
...
def load_ausentismo_data():
    possible_sheets = [
        "Ausentismo", "AUSENTISMO", "Ausentismo_Normalizado",
        "ausentismo", "Licencias", "Licencias_Normalizado"
    ]
    for sheet_name in possible_sheets:
        df = try_load_sheet(SPREADSHEET_URL, worksheet=sheet_name)
        if df is not None and not df.empty:
            df = normalize_columns(df)
            col_fecha = first_existing_column(df, ["fecha", "fecha_licencia", "fecha_inicio", "fecha_desde"])
            col_dias = first_existing_column(df, ["cantidad_dias", "dias", "dias_corridos", "dias_filtrados"])
            col_tipo = first_existing_column(df, ["tipo_licencia", "motivo_licencia", "licencia", "motivo"])
            col_nombre = first_existing_column(df, ["apellido_y_nombre", "apellidoynombre", "nombre_y_apellido"])
            col_area = first_existing_column(df, ["area", "area_"])
            col_sector = first_existing_column(df, ["sector"])
            col_localidad = first_existing_column(df, ["localidad"])
...
def build_dotacion_history(fecha_fin, area=None, localidad=None, online=None):
    ...
    for f in fechas:
        ...
        rows.append({"periodo": f.strftime("%Y-%m"), "dotacion": len(df_aux)})
...
def build_rotacion_history(fecha_fin, area=None, localidad=None):
    ...
    for f in fechas:
        ...
        rows.append({"periodo": inicio.strftime("%Y-%m"), "rot_real": info["rot_real"], "rot_tasa": info["rot_tasa"]})
    return pd.DataFrame(rows)
...
def filtrar_ausentismo_mes(anio, mes, area=None, localidad=None):
    ...
    df = df_aus.copy()
    df = df[df[col_fecha].notna() & (df[col_fecha].dt.year == anio) & (df[col_fecha].dt.month == mes)].copy()
...
st.markdown(
    ...
    <div class="sticky-header-container">
        <div style="display:flex; justify-content:space-between; align-items:flex-start; gap:20px;">
            <div>
                <h1 style="margin:0; padding:0; font-size:34px;">Dashboard de Indicadores RRHH - Autolux</h1>
            ...
            <div style="display:flex; flex-direction:column; align-items:flex-end; min-width:110px;">
                <img src="https://upload.wikimedia.org/wikipedia/commons/e/ee/Toyota_logo_%28Red%29.svg" style="height:45px; object-fit:contain; margin-bottom:3px;">
                <div style="font-weight:800; color:#58595B; font-size:15px; letter-spacing:1px;">Autolux</div>
            </div>
        </div>
    </div>
    """,
...
with tabs[0]:
    ...
    if col_area and col_sector and col_puesto and col_localidad and not df_snap.empty:
        ...
        st.warning("No se puede construir la matriz. Revisá Área, Sector, Puesto, Localidad o que existan filas para el período filtrado.")
    ...
    with c1:
        ...
        if selected_puesto:
            st.caption(f"Filtro: {selected_area} > {selected_sector} > {selected_puesto}")
        elif selected_sector:
            st.caption(f"Filtro: {selected_area} > {selected_sector}")
        elif selected_area:
            st.caption(f"Filtro: {selected_area}")
        else:
    ...
    with col_g2:
        ...
        if not cat_df.empty:
            fig_cat = px.bar(cat_df, x="cantidad", y="categoria", orientation="h", text_auto=True)
    ...
    cols_to_show = [c for c in [col_nombre, col_puesto, col_sector, col_area, col_localidad] if c and c in df_estructura_filtrado.columns]
    if cols_to_show:
        rename_map = {
...
with tabs[1]:
    ...
    with l1:
        if col_area and not df_snap_tasa.empty:
            ...
            resumen_area = tasa_area.pivot_table(index="area", columns="flag_online_tasa", values="cantidad", fill_value=0).reset_index()
            ...
            fig1 = px.pie(resumen_area, names="area", values="% Declarados", hole=0.55, title="% Declarados Online por Área")
            ...
            fig2 = px.pie(resumen_area, names="area", values="% No Declarados", hole=0.55, title="% No Declarados por Área")
            fig2.update_layout(height=300, margin=dict(l=10, r=10, t=45, b=10))
            st.plotly_chart(fig2, use_container_width=True)

    with l2:
        comp_area = build_tasa_area_snapshot(fecha_corte, localidad=None if sel_localidad == "Todas" else sel_localidad)
        ...
        if col_nombre and col_nombre in df_online.columns:
            ...
            st.dataframe(df_online[cols_show].sort_values(by=col_nombre).reset_index(drop=True), use_container_width=True, height=260)
...
with tabs[2]:
    ...
    with b1:
        df_area_rot = build_rotacion_area_mes(fecha_inicio_mes, fecha_corte, localidad=None if sel_localidad == "Todas" else sel_localidad)
        if not df_area_rot.empty:
            fig_area_real = make_bar_chart(df_area_rot.sort_values("rot_real", ascending=False), x="area", y="rot_real", title="% Rotación Mensual por Área", color=COLOR_GRIS)
            st.plotly_chart(fig_area_real, use_container_width=True)
    with b2:
        df_rot_hist = build_rotacion_history(fecha_corte, area=None if sel_area == "Todas" else sel_area, localidad=None if sel_localidad == "Todas" else sel_localidad)
        if not df_rot_hist.empty:
            fig_real_hist = make_bar_chart(df_rot_hist, x="periodo", y="rot_real", title="% Rotación Real - Evolución Mensual", color=COLOR_ROJO)
            st.plotly_chart(fig_real_hist, use_container_width=True)
    with b3:
        if 'df_rot_hist' in locals() and not df_rot_hist.empty:
            fig_tasa_hist = make_bar_chart(df_rot_hist, x="periodo", y="rot_tasa", title="% Rotación TASA - Evolución Mensual", color=COLOR_ROJO)
    ...
    with c1:
        ...
        if col_motivo_baja and not bajas_mes.empty:
            ...
            fig_mot = px.bar(motivo_df, x="cantidad", y="motivo_baja", orientation="h", text_auto=True)
            fig_mot.update_traces(marker_color=COLOR_ROJO)
            fig_mot.update_layout(height=300, margin=dict(l=10, r=10, t=20, b=10), xaxis_title="", yaxis_title="")
...
with tabs[3]:
    ...
    if not ausentismo_disponible():
        st.warning("No encontré una hoja de ausentismo compatible. La app está preparada para leer hojas llamadas 'Ausentismo', 'AUSENTISMO' o 'Ausentismo_Normalizado'.")
    else:
        ...
        with aa2:
            if not df_aus_mes.empty and col_nombre_aus:
                ...
                st.dataframe(df_aus_mes[resumen_cols].reset_index(drop=True), use_container_width=True, height=160)

        bb1, bb2, bb3 = st.columns([1.2, 1.2, 1.3])
        with bb1:
            if not df_aus_mes.empty and col_sector_aus:
                sec_df = df_aus_mes.groupby(col_sector_aus)[col_dias_aus].sum().reset_index().sort_values(col_dias_aus, ascending=False)
                sec_df["aus_pct"] = sec_df[col_dias_aus].apply(lambda x: (x / (dot_mes * dias_lab) * 100) if dot_mes > 0 and dias_lab > 0 else 0)
                fig_sec = px.bar(sec_df, x="aus_pct", y=col_sector_aus, orientation="h", text_auto=".2f")
                fig_sec.update_traces(marker_color=COLOR_ROJO)
                fig_sec.update_layout(title="% Ausentismo por Sector", height=300, margin=dict(l=10, r=10, t=45, b=10), xaxis_title="", yaxis_title="")
                st.plotly_chart(fig_sec, use_container_width=True)
        with bb2:
            if not df_aus_mes.empty and col_area_aus:
                area_df = df_aus_mes.groupby(col_area_aus)[col_dias_aus].sum().reset_index().sort_values(col_dias_aus, ascending=False)
                area_df["aus_pct"] = area_df[col_dias_aus].apply(lambda x: (x / (dot_mes * dias_lab) * 100) if dot_mes > 0 and dias_lab > 0 else 0)
                fig_area_aus = px.bar(area_df, x=col_area_aus, y="aus_pct", text_auto=".2f")
                fig_area_aus.update_traces(marker_color=COLOR_ROJO)
                fig_area_aus.update_layout(title="% Ausentismo por Área", height=300, margin=dict(l=10, r=10, t=45, b=10), xaxis_title="", yaxis_title="")
                st.plotly_chart(fig_area_aus, use_container_width=True)
        with bb3:
            punto_df = pd.DataFrame({"mes": [sel_mes_nombre], "ausentismo": [ausentismo_pct]})
            fig_punto = px.scatter(punto_df, x="mes", y="ausentismo", size=[18])
            fig_punto.update_traces(marker=dict(color=COLOR_ROJO))
            fig_punto.update_layout(title="Ausentismo Mensual", height=300, margin=dict(l=10, r=10, t=45, b=10), xaxis_title="", yaxis_title="")
            st.plotly_chart(fig_punto, use_container_width=True)

        cc1, cc2 = st.columns([1.5, 1.2])
        with cc1:
            if not df_aus_mes.empty and col_tipo_aus:
                tipo_df = df_aus_mes.groupby(col_tipo_aus)[col_dias_aus].sum().reset_index().sort_values(col_dias_aus, ascending=False)
                tipo_df["aus_pct"] = tipo_df[col_dias_aus].apply(lambda x: (x / (dot_mes * dias_lab) * 100) if dot_mes > 0 and dias_lab > 0 else 0)
                fig_tipo = px.bar(tipo_df, x="aus_pct", y=col_tipo_aus, orientation="h", text_auto=".2f")
                fig_tipo.update_traces(marker_color=COLOR_ROJO)
                fig_tipo.update_layout(title="% Ausentismo por Tipo de Licencia", height=320, margin=dict(l=10, r=10, t=45, b=10), xaxis_title="", yaxis_title="")
